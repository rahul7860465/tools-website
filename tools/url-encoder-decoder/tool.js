...
encodeBtn.addEventListener("click", () => {
  try {
    ...
    if (encoded.length > MAX_OUTPUT_CHARS) {
      output.value = "";
      return show("Encoded output too large to display.", true);
    }
    output.value = encoded;
    show("Encoded.");
    window.ToolboxTracking?.trackRun("url-encoder-decoder", "encode");
  } catch (e) {
    output.value = "";
    show(e instanceof Error ? e.message : "Failed to encode.", true);
  }
});
...
decodeBtn.addEventListener("click", () => {
  try {
    ...
    const res = safeExec(() => decodeURIComponent(raw));
    if (!res.ok) {
      output.value = "";
      return show("Invalid encoded string.", true);
    }
    ...
    show("Decoded.");
    window.ToolboxTracking?.trackRun("url-encoder-decoder", "decode");
  } catch (e) {
    output.value = "";
    show(e instanceof Error ? e.message : "Failed to decode.", true);
  }
});
